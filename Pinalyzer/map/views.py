...
from models import PinModel, CategoryModel, UserModel, UserStatModel, SocialUserModel
...
def get_score(request):
	if request.is_ajax() and request.method == 'POST':
		...
		prev_stat, stat=user.get_stat_evolution()
		if prev_stat:
		...
		try:
			...
			pass
		res = {'status':'OK', 'data':{'user':user, 'stat': stat,'prev_stat':prev_stat, 'score':score, 'prev_score':prev_score, 'history':history, 'last_history':history[-7:],'info_open_graph':info_open_graph }, }
